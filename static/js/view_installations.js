...
function filterTableByColumns() {
  ...
  let filtered = (window.installations || []).filter(inst => {
    let match = true;
    if (filters.clientId && !(String(inst.clientId || "").toLowerCase().includes(filters.clientId))) match = false;
    if (filters.clientName && !(String(inst.clientName || "").toLowerCase().includes(filters.clientName))) match = false; 
    if (filters.plan && !(String(inst.plan || "").toLowerCase().includes(filters.plan))) match = false;
    if (filters.filial && !(String(inst.filial || "").toLowerCase().includes(filters.filial))) match = false;
    if (filters.dueDate) {
    ...
    if (filters.requestDate && !(String(inst.requestDate || "").toLowerCase().includes(filters.requestDate))) match = false;
    if (filters.attendant && !(String(inst.attendant || "").toLowerCase().includes(filters.attendant))) match = false;
    if (filters.turno_preferencial && !(String(inst.turno_preferencial || "").toLowerCase().includes(filters.turno_preferencial))) match = false;
    
    // Aplicar filtro de filial
    if (filialFilter && String(inst.filial || "").toLowerCase() !== filialFilter.toLowerCase()) {
      match = false;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const url = window.API_URLS ? window.API_URLS.getInstallations : '/api/installations';
  ...
  fetch(url, {
    ...
    .then((data) => {
      ...
      if (data.success) {
        console.log('Carregando instalações:', data.installations.length, 'registros'); // Debug
        window.installations = data.installations;
        paginatedData = data.installations;
        loadInstallations();
        renderPaginationControls();
      } else {
        const message = "Erro ao carregar instalações: " + (data.message || "Erro desconhecido.");
  ...
  if (btnLimparFiltro) {
    btnLimparFiltro.addEventListener("click", () => {
      document.getElementById("filterIdCliente") && (document.getElementById("filterIdCliente").value = "");
      filterInputs.forEach(f => {
...
function renderInstallationsTable() {
  ...
  pageData.forEach((installation) => {
    ...
    } else if (plano.includes('300mb') || plano.includes('200mb') || plano.includes('100mb') || plano.includes('70mb') || plano.includes('50mb') || plano.includes('30mb')) {
      badgePlanClass = "badge-plan-orange";
    }
    ...
    let obsIndicator = "";
    if (installation.observation && installation.observation.trim().length > 0) {
      obsIndicator = '<span title="Possui observação" style="color:#f59e42; margin-left:4px;"><i class="fas fa-sticky-note"></i></span>';
    }
    row.innerHTML = `
...
function openEditModal(installation) {
  ...
  document.getElementById('editTurno').value = installation.turno_preferencial || '';
    ...
    if (typeof flatpickr !== 'undefined') {
      ...
      if (dueDateInput) {
        ...
        Promise.all([
          ...
          const defaultDate = inputValue ? parseDateSafe(inputValue) : (installation.dueDate ? parseDateSafe(installation.dueDate) : new Date());
          ...
          flatpickr("#editDueDate", {
            ...
            disable: [
              ...
              function(date) {
                ...
                return limitesFilial['dom'] === undefined && date.getDay() === 0;
              }
            ],
              locale: {
                firstDayOfWeek: 1,
...
async function handleEditSubmit(e) {
  ...
  const rawDueDate = (dueDateInput && dueDateInput.value ? dueDateInput.value.trim() : '');
  if (rawDueDate) {
  ...
  if (!dueDateFormatted && dueDateInput && dueDateInput._flatpickr && dueDateInput._flatpickr.selectedDates.length) {
  ...
  try {
    ...
    if (result.success) {
      ...
      const installationIndex = window.installations.findIndex(inst => inst.id === currentEditInstallation.id);
      if (installationIndex !== -1) {
        window.installations[installationIndex] = { ...window.installations[installationIndex], ...updateData };
      }
