document.addEventListener('DOMContentLoaded', function() {
  ...
  function preencherCampos(limites) {
    filiais.forEach(filial => {
      dias.forEach(dia => {
        ...
        if (input) {
          input.value = (limites && limites[filial] && typeof limites[filial][dia] !== 'undefined') ? limites[filial][dia] : 20;
        }
      });
    });
  }
  ...
  function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const hasPersonalizado = limitesPersonalizados[selectedFilial] && limitesPersonalizados[selectedFilial].hasOwnProperty(dateStr);
      const limitePersonalizado = hasPersonalizado ? parseInt(limitesPersonalizados[selectedFilial][dateStr], 10) : null;
      
      // Obter limite padrão
      const diasSemana = ['dom', 'seg', 'ter', 'qua', 'qui', 'sex', 'sab'];
      const diaSemanaKey = diasSemana[dayOfWeek];
      const limitePadrao = (config.limite_diario && config.limite_diario[selectedFilial] && config.limite_diario[selectedFilial][diaSemanaKey]) || 20;
      ...
      // Contar instalações
      const count = installationsData[selectedFilial] && installationsData[selectedFilial][dateStr] ? installationsData[selectedFilial][dateStr] : 0;
      ...
      html += `<td class="calendar-day" data-date="${dateStr}" style="background: ${bgColor}; border: 2px solid ${borderColor}; cursor: pointer;" title="Limite: ${limite} | Agendados: ${count}${hasPersonalizado ? ' (Personalizado)' : ''}">`;
  ...
  function openDayModal(dateStr) {
    ...
    modalDate.textContent = dateFormatted.charAt(0).toUpperCase() + dateFormatted.slice(1);
  ...
  function saveDayLimit(limit) {
    ...
    config.limites_personalizados[selectedFilial][selectedDate] = parseInt(limit, 10);
    
    fetch('/api/configuracoes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ limites_personalizados: config.limites_personalizados })
    })
    .then(res => res.json())
    .then(data => {
      if (data.success) {
        loadConfig().then(() => {
          renderCalendar();
          closeDayModal();
          showMessage('msgLimitePersonalizado', 'Limite personalizado salvo com sucesso!', 'success');
        });
      } else {
        showMessage('msgLimitePersonalizado', data.message || 'Erro ao salvar.', 'error');
      }
    })
    .catch(() => {
  ...
  function removeDayLimit() {
    if (!selectedDate || !selectedFilial) {
      showMessage('msgLimitePersonalizado', 'Data ou filial não selecionada.', 'error');
      return;
    }
    
    if (!config.limites_personalizados || !config.limites_personalizados[selectedFilial]) {
      showMessage('msgLimitePersonalizado', 'Não há limite personalizado para remover.', 'error');
      return;
    }
    
    if (!config.limites_personalizados[selectedFilial].hasOwnProperty(selectedDate)) {
      showMessage('msgLimitePersonalizado', 'Não há limite personalizado para esta data.', 'error');
      return;
    }
    ...
    .then(data => {
      if (data.success) {
        ...
        if (config.limites_personalizados[selectedFilial]) {
          ...
          if (Object.keys(config.limites_personalizados[selectedFilial]).length === 0) {
        ...
        loadConfig().then(() => {
          renderCalendar();
          closeDayModal();
          showMessage('msgLimitePersonalizado', 'Limite personalizado removido com sucesso!', 'success');
        });
      } else {
        showMessage('msgLimitePersonalizado', data.message || 'Erro ao remover.', 'error');
      }
    })
    .catch(error => {
      console.error('Erro ao remover limite:', error);
      showMessage('msgLimitePersonalizado', 'Erro ao remover limite personalizado.', 'error');
    });
  }
  ...
  if (btnRemoveDayLimit) {
    btnRemoveDayLimit.addEventListener('click', function(e) {
      e.preventDefault();
      if (confirm('Tem certeza que deseja remover a personalização deste dia?')) {
        removeDayLimit();
      }
    });
  }
  ...
  if (inputFeriados) {
    ...
    // Salvar feriados
    document.getElementById('formFeriados').addEventListener('submit', function(e) {
      e.preventDefault();
      fetch('/api/configuracoes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (btnPesquisar) {
    btnPesquisar.addEventListener('click', function() {
      ...
      if (!pesquisaId && !pesquisaNome) {
        mostrarMensagemPesquisa('Por favor, digite o ID ou nome do cliente.', 'error');
        return;
      }
  ...
  fetch(url)
    ...
    .then(data => {
      if (data.success) {
        ...
        mostrarMensagemPesquisa(`Serviço encontrado para "${searchTerm}"!`, 'success');
      } else {
        const searchTerm = clientId || clientName;
        mostrarMensagemPesquisa(data.message || `Serviço não encontrado para "${searchTerm}".`, 'error');
        ocultarFormularioEdicao();
      }
    })
    .catch(error => {
...
function preencherFormularioEdicao(installation) {
  ...
  document.getElementById('editClientName').value = installation.clientName || '';
  document.getElementById('editInstallationType').value = installation.installationType || '';
  document.getElementById('editPlan').value = installation.plan || '';
  document.getElementById('editRequestDate').value = installation.requestDate || '';
  
  // Converter dueDate para o formato correto de exibição
  const dueDateFormatted = formatDateForDisplayConfig(installation.dueDate || '');
  ...
  document.getElementById('editObservation').value = installation.observation || '';
...
function salvarEdicaoServico() {
  ...
  .then(data => {
    if (data.success) {
      ...
      mostrarMensagemEdicao(data.message || 'Erro ao atualizar serviço.', 'error');
    }
  })
  .catch(error => {
