...
function deleteInstallation(id) {
  Swal.fire({
    ...
    inputPlaceholder: "Informe o motivo da exclusão (obrigatório)",
    inputAttributes: {
      "aria-label": "Informe o motivo da exclusão",
    },
    inputValidator: (value) => {
      if (!value || !value.trim()) {
        return "O motivo da exclusão é obrigatório.";
      }
    },
    ...
    button.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (data.success && data.user) {
          ...
          showNotification(
            "Erro ao carregar usuário. Dados inválidos.",
            "error",
          );
        }
      } catch (error) {
